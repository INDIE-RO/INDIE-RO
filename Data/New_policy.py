...
db = pymysql.connect(
		host='indiero.checo2o802an.ap-northeast-2.rds.amazonaws.com',            # 접속할 mysql server의 주소
		port=3306,        # 접속할 mysql server의 포트 번호
		user='root',     
		passwd='sync2024!',
		db='indiero',         # 접속할 database명
...
# 데이터를 저장할 빈 리스트 초기화
data = []
all_policy_ids = set(existing)  # 기존 정책 ID를 포함한 집합
...
# 중복 컬럼 제거
#df = df.drop_duplicates(subset=['정책명', '정책소개', '지원내용', '기간유형', '신청기간', '신청시작날짜', '신청마감날짜',
         #'연령정보', '연령코드', '정책분야', '지역', '거주지및소득조건', '참여제한대상', '추가단서사항',
         #'학력요건', '전공요건', '취업상태', '제출서류', '연락처', '신청사이트주소'])
...
dt = {'id': sa.types.BIGINT(),
      ...
      'views': sa.types.INT(),
      's1_id': sa.types.INT(),
      's1_title': sa.types.VARCHAR(255),
      's1_score': sa.types.FLOAT(),
      's2_id': sa.types.INT(),
      's2_title': sa.types.VARCHAR(255),
